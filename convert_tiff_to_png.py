...
def convert_tiff_to_png(input_path, output_path=None, sigmoid_k=0.001):
...
def convert_directory(input_dir, output_dir=None, sigmoid_k=0.001, pattern="*.tiff"):
    """Convert all TIFF files in a directory to PNG.
    ...
    for tiff_file in tiff_files:
        ...
        if convert_tiff_to_png(tiff_file, output_file, sigmoid_k):
            success_count += 1
        else:
            fail_count += 1
...
def main():
    ...
    if input_path.is_dir():
        convert_directory(input_path, args.output, args.sigmoid_k, args.pattern)

    # Handle single file conversion
    elif input_path.is_file():
        convert_tiff_to_png(input_path, args.output, args.sigmoid_k)
